...
const ClienteForm = ({ onClienteAgregado }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.post('http://localhost:5000/api/clientes', formData)
      .then(response => {
  ...
  return (
    <Box component="form" onSubmit={handleSubmit} mb={2} sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
      <TextField label="Nombre" name="nombre" value={formData.nombre} onChange={handleChange} required />
      <TextField label="Integrantes" name="integrantes" type="number" value={formData.integrantes} onChange={handleChange} />
      <TextField label="Número de Recibo" name="numeroRecibo" value={formData.numeroRecibo} onChange={handleChange} required />
      <TextField
