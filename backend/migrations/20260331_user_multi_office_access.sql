-- Migracion segura e idempotente para permitir que un usuario
-- tenga acceso a varias oficinas y cambie entre ellas.
...
CREATE TABLE IF NOT EXISTS usuario_oficinas (
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'usuario_oficinas_usuario_id_fkey'
          AND conrelid = 'usuario_oficinas'::regclass
    ) THEN
        ALTER TABLE usuario_oficinas
            ADD CONSTRAINT usuario_oficinas_usuario_id_fkey
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'usuario_oficinas_oficina_id_fkey'
          AND conrelid = 'usuario_oficinas'::regclass
    ) THEN
        ALTER TABLE usuario_oficinas
            ADD CONSTRAINT usuario_oficinas_oficina_id_fkey
            FOREIGN KEY (oficina_id)
            REFERENCES oficinas(id)
            ON DELETE CASCADE;
    END IF;
...
CREATE INDEX IF NOT EXISTS idx_usuario_oficinas_oficina_id
    ON usuario_oficinas (oficina_id);
